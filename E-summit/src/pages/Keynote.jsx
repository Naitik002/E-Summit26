...
const MultiThemeGallery = () => {
  return (
    <div className="min-h-screen bg-black overflow-x-hidden font-sans scroll-smooth">
      ...
      <section
  className="relative py-16 md:py-24 bg-[#020617] bg-cover bg-center bg-no-repeat overflow-hidden"
...
>
    ...
    <div className="flex flex-col lg:flex-row justify-between items-center lg:items-end gap-10 mb-12 md:mb-16">
      <div className="w-full lg:max-w-2xl text-center lg:text-left">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} className="flex items-center justify-center lg:justify-start gap-3 text-cyan-500 mb-4 font-mono uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] md:text-xs">
          <div className="hidden sm:block w-8 md:w-12 h-[1px] bg-cyan-500 animate-pulse" /> [ Accessing Intel ]
        </motion.div>

        <h2 className="text-4xl sm:text-5xl md:text-6xl font-black italic text-white uppercase tracking-tighter leading-tight relative">
          Keynote <span className="text-cyan-500 underline decoration-cyan-500/30">Sessions</span>
          {/* Subtle glitch shadow */}
          <span className="absolute -top-1 -left-1 text-cyan-500/10 blur-[2px] select-none">Keynote Sessions</span>
        </h2>

        <div className="bg-slate-900/40 border border-cyan-500/20 p-4 md:p-5 rounded-sm backdrop-blur-md relative mt-6 md:mt-8 mx-auto lg:mx-0 max-w-lg lg:max-w-none shadow-[0_0_30px_rgba(0,0,0,0.5)]">
          <div className="absolute -top-px left-1/2 -translate-x-1/2 lg:translate-x-0 lg:left-10 w-20 h-[2px] bg-cyan-400 shadow-[0_0_10px_#22d3ee]" />
          {/* Decorative Corner */}
          <div className="absolute top-0 right-0 w-4 h-4 border-t border-r border-cyan-500/40" />
          
          <div className="text-slate-300 font-mono text-xs md:text-sm leading-relaxed">
      ...
      <motion.div
        ...
        className="text-cyan-400/20 lg:text-cyan-400/40 drop-shadow-[0_0_20px_rgba(34,211,238,0.2)] relative"
      >
        <Gamepad2 size={120} className="md:w-[200px] md:h-[200px] lg:w-[250px] lg:h-[250px]" strokeWidth={2.5} />
        {/* Floating crosshair around controller */}
        <div className="absolute inset-0 border border-cyan-500/10 rounded-full scale-125 animate-ping opacity-20" />
      </motion.div>
    </div>

    {/* Keynote Marquee */}
    <div className="relative flex overflow-hidden py-8 md:py-10 -mx-4 md:mx-0">
      {/* 3. MARQUEE HUD ELEMENTS (Brackets) */}
      <div className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-cyan-500/30 z-20 pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-cyan-500/30 z-20 pointer-events-none" />
        ...
        {infiniteKeynotes.map((s, idx) => (
          <div key={idx} className="w-[260px] md:w-[320px] group flex-shrink-0 cursor-crosshair">
            <div className="bg-slate-900 border border-slate-800 group-hover:border-cyan-500/50 p-1 clip-path-gaming transition-all duration-500 shadow-xl relative overflow-hidden">
              ...
              <div className="absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.2)_50%)] bg-[size:100%_4px] pointer-events-none z-10 opacity-0 group-hover:opacity-100 transition-opacity" />
              ...
              <div className="relative overflow-hidden aspect-[3/4]">
                ...
                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                   <div className="text-[8px] font-mono text-cyan-400 bg-black/80 px-1 border border-cyan-500/30">ID_{idx + 100}</div>
                </div>

                <div className="absolute bottom-3 left-3 md:bottom-4 md:left-4">
                  <span className="bg-cyan-600 text-[8px] md:text-[10px] px-2 py-1 font-bold text-black uppercase shadow-[2px_2px_0px_#000]">{s.power}</span>
                </div>
              </div>
              <div className="p-3 md:p-4 flex justify-between items-center bg-slate-950/80 border-t border-slate-800">
                <h3 className="text-white text-sm md:text-base font-bold tracking-tight italic uppercase group-hover:text-cyan-400 transition-colors">{s.name}</h3>
                <div className="text-cyan-500 group-hover:scale-125 transition-transform">{s.icon}</div>
              </div>
            </div>
          </div>
        ))}
  ...
  {/* --- FOREGROUND ELEMENT --- */}
  {/* 4. Foreground Blur Particle */}
  <div className="absolute -bottom-10 -left-10 w-64 h-64 bg-cyan-500/10 rounded-full blur-[100px] pointer-events-none" />
</section>
